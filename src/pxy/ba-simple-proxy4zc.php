...
if ( !$url ) {
        ...
        if ( strtolower($_SERVER['REQUEST_METHOD']) == 'post' ) {
                ...
                $post_data = $_POST;
                if (!is_string($post_data)) { $post_data = http_build_query($post_data); }
                curl_setopt( $ch, CURLOPT_POSTFIELDS, $post_data );
        }
  
        if ( array_key_exists('send_cookies', $_GET) && $_GET['send_cookies'] ) {
                $cookie = array();
        ...
        curl_setopt( $ch, CURLOPT_USERAGENT, (array_key_exists('user_agent', $_GET) && $_GET['user_agent']) ? $_GET['user_agent'] : $_SERVER['HTTP_USER_AGENT'] );
...
if ( array_key_exists('mode', $_GET) && ($_GET['mode'] == 'native') ) {
        ...
        foreach ( $header_text as $header ) {
                if ( preg_match( '/^(?:Content-Type|Content-Language|Set-Cookie):/i', $header ) ) {
                        header( $header );
                        writeStdErr("header=".$header);
                }
        }
  
        print $contents;
        writeStdErr("contents=" . substr($contents, 0, 300));
        ...
        if ( array_key_exists('full_headers', $_GET) && $_GET['full_headers'] ) {
                $data['headers'] = array();
        ...
        if ( array_key_exists('full_status', $_GET) && $_GET['full_status'] ) {
                $data['status'] = $status;
        } else {
                $data['status'] = array();
        ...
        $is_xhr = array_key_exists('HTTP_X_REQUESTED_WITH', $_SERVER) && (strtolower($_SERVER['HTTP_X_REQUESTED_WITH']) == 'xmlhttprequest');
