...
Zotero.Translators = new function() {
	...
	this._getCode = function(translatorID, callback) {
		...
		Zotero.Connector.callMethod("getTranslatorCode", {"translatorID":translatorID}, function(result) {
			...
			var translatorUrl = typeof zscrapTranslatorURL === "function" ? zscrapTranslatorURL(translatorID) : ZOTERO_CONFIG.REPOSITORY_URL+"/code/"+ZOTERO_CONFIG.REPOSITORY_CHANNEL+"/"+translatorID;
			Zotero.HTTP.doGet(translatorUrl, function(xmlhttp) {
	...
	this.getWebTranslatorsForLocation = function(uri, callback) {
		...
		Zotero.Connector.callMethod("getTranslators", {"url":uri}, function(result) {
			if(result) {
				...
				// fix-me
				if (typeof zscrapGetTranslators === "function") {
					var searchURI = fetchURIs.shift();
					// fix-me: copy from getTranslatorsFromRepo:
					var foundTranslators = zscrapGetTranslators(searchURI);
					for(var i=0; i<foundTranslators.length; i++) {
						if(!(foundTranslators[i].translatorType & TRANSLATOR_TYPES.web)) {
							foundTranslators.splice(i, 1);
						}
