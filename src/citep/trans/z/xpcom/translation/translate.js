...
Zotero.Translate.Base.prototype = {
	...
	"incrementAsyncProcesses":function(f) {
		this._runningAsyncProcesses++;
                console.log("translate.js incrementAsyncProcesses", this._runningAsyncProcesses);
	...
	"decrementAsyncProcesses":function(f, by) {
		this._runningAsyncProcesses -= (by ? by : 1);
                console.log("translate.js decrementAsyncProcesses", by, this._runningAsyncProcesses);
		if(!this._parentTranslator) {
	...
	"_getTranslatorsTranslatorsReceived":function(allPotentialTranslators, properToProxyFunctions) {
		...
		for(var i=0, n=allPotentialTranslators.length; i<n; i++) {
			...
			if(true || translator.runMode === Zotero.Translator.RUN_MODE_IN_BROWSER) { // fix-me
		...
		if(this._waitingForRPC) {
			var me = this;
			var myCookie = typeof this.document.body.dataset.cookie !== "undefined" ?
				this.document.body.dataset.cookie : this.document.cookie;
			var myUri = typeof this.document.body.dataset.locationHref !== "undefined" ?
				this.document.body.dataset.locationHref : this.location.toString();
			...
			Zotero.Connector.callMethod("detect",
						    {
							    ...
							    "uri": myUri,
							    // "cookie": this.document.cookie, // fix-me
							    "cookie": myCookie,
	...
	"_saveItems":function(items) {
                console.log("translate.js saveItems", items);
		var me = this,
	...
	"_detectTranslatorLoaded":function() {
		...
		try {
			...
			return;
                ...
                console.log("detectWeb => returnValue", returnValue);
	...
	"_generateSandbox":function() {
		...
		if (this._parentTranslator && this._parentTranslator._sandboxManager.newChild) {
			...
			this._sandboxManager = new Zotero.Translate.SandboxManager(this._sandboxLocation);
                        this_sandboxManager = this._sandboxManager; // fix-me: we have doGet, doPost, or?.. NO
		}
		...
		this._sandboxManager.importObject(this.Sandbox, this);
                // thisSandBox = this.Sandbox; // fix-me, no doGet or doPost here
                // console.log("---------------- before importObject Utilities");
		this._sandboxManager.importObject({"Utilities":new Zotero.Utilities.Translate(this)});
                // myNewZUTrans = new Zotero.Utilities.Translate(this); // fix-me, ["_translate"]

		this._sandboxZotero = this._sandboxManager.sandbox.Zotero;
		// thissandboxZotero = this._sandboxZotero; // fix-me, here we get doGet, doPost
                // fixMe(); // fix-me
		...
		this._sandboxZotero.Utilities.HTTP = this._sandboxZotero.Utilities;
                thisSbxZUtHTTP  = this._sandboxZotero.Utilities.HTTP;
		...
		this._sandboxManager.sandbox.ZU = this._sandboxZotero.Utilities;
                // console.log("translate.js sandbox.ZU", this._sandboxManager.sandbox.ZU.toString());
 ...
 */
// fix-me Zotero.Translate.Web.prototype.setDocument = function(doc) {
Zotero.Translate.Web.prototype.setDocument = function(doc) {
	this.document = doc;
	var locationHref = doc.location ? doc.location.href : doc.body.dataset.locationHref;
	this.setLocation(locationHref);
}
