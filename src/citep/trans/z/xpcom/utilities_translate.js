...
Zotero.Utilities.Translate.prototype.doGet = function(urls, processor, done, responseCharset) {
	console.log("http.js|utilities_translate.js doGet", "imported", Zotero.HTTP.doGet.isImported, url);
	var callAgain = false,
		me = this,
...
Zotero.Utilities.Translate.prototype.doPost = function(url, body, onDone, headers, responseCharset) {
	// fix-me: This is the one used in translators. Is resolveURL supposed to fix the CORS problem??
	console.log("http.js|utilities_translate.js doPost", "imported", Zotero.HTTP.doGet.isImported, url);
	var translate = this._translate;
	url = translate.resolveURL(url);
	url = addCORSproxyIfNeeded(url);
